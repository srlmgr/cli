...
func (r *setupRunner) setupSeasonList(
	...
	for i := range seasons {
		...
		useDefaultJoinedAt := seasons[i].DefaultJoinedAt
		if useDefaultJoinedAt == "" {
			useDefaultJoinedAt = seasons[i].StartsAt
			if useDefaultJoinedAt == "" {
				useDefaultJoinedAt = time.Now().Format(time.DateOnly)
			}
		}
...
func (r *setupRunner) setupTeamMembers(
	...
	for i := range drivers {
		if drivers[i].JoinedAt == "" {
			drivers[i].JoinedAt = defaultJoinedAt
		}
	}
	if err := r.setTeamMembers(ctx, teamID, drivers); err != nil {
...
func (r *setupRunner) setupModelVariants(
	...
	for i := range modelVariants {
		modelVariant, created, err := r.ensureCarModelVariant(ctx, modelID, modelVariants[i].Name)
		if err != nil {
		...
		r.carModelVariantLookup[modelVariants[i].Name] = modelVariant
		modelVariantIDs[modelVariants[i].Name] = modelVariant.GetId()
	}
