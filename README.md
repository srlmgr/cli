...
[![Build Status](https://img.shields.io/github/checks-status/srlmgr/cli/main?color=black&style=for-the-badge&logo=github)][github-actions]
...
```sh
git clone https://github.com/srlmgr/cli
cd cli
```
...
After cloning the repository you already changed into the project dir cli.
...
This section will guide you to setup a fully-functional local copy of `cli`
...
from [`cli's` Makefile][makefile-file] (most of which are
OS-independent)!

### Installing dependencies

To install all dependencies associated with `cli`, run the
command
...
Tests in `cli` are classified as _fast_ and _slow_ - depending
...
`cli` is powered by a template generated using [`cookiecutter-go`][cookiecutter-go-link] which is based on [`go-template`][go-template-link]
